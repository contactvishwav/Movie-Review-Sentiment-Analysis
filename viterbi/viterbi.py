...
# checking if file exists, debugger is being weird
# filename = 'ner.dev'
# file_path = os.path.abspath(filename)
# if os.path.isfile(file_path):
#     print(f"File {filename} FOUND! good to go")
# else:
#     print(f"File {filename} not there, filepath: {file_path}")

def decode(input_length: int, tagset: List[str], score: Callable[[str, str, int], float]) -> List[str]:
    """
    ...
    #input breakdown:
    # input length = basically just the number of positions in the seq that we process via viterbi
    # tagset = our y axis, just every tag in the viterbi table
    # score - important, since 1-indexed, we must adjust accordingly
    # takes in current position's tag, a tag for the previous position, and the current 'word' or in this case,
    # position since we don't know the exact word, 


    V = [{}] # initialize viterbi table
    backpointer = [{}] # making a nice backpointer table for the best previous tags for each tag
    
    # adding start tag bc no start tags included
    for tag in tagset:
        # initial start tag prob to tag at position 1 is 1 bc start tag is always going to be S, 100%. 
        V[0][tag] = score(tag, '<START>', 1)
        backpointer[0][tag] = '<START>' #self explanatory, tag just goes back
    
    #main meat of the code
    # run viterbi for the positions from 2 to input_length-2 excluding <START> and <STOP>
    # cuz start stop already accounted for, above and below this loop
    # print('populating cells')
    for i in range(2, input_length - 1):
        V.append({})
        backpointer.append({})
        for currentTag in tagset:
            maxScore = float('-inf') #as suggested by Brendan King's section
            bestPrev = None
            for prevTag in tagset:
                # 65:03 in video
                # i-2 bc we are 1 indexed thanks to the start tag, so we use i-2 to get the real i-1 prev tag
                currentScore = V[i-2][prevTag] + score(currentTag, prevTag, i)
                if currentScore > maxScore:
                    # no need to hang onto all scores, just update lmao
                    maxScore = currentScore
                    bestPrev = prevTag
            #store winning score here :)
            # best score for current tag at i-1 which is technically position 'i' in the seq
            V[i-1][currentTag] = maxScore
            backpointer[i-1][currentTag] = bestPrev
    
    # termination step, same as above loop but cooler and also not a loop
    # we just need a special ending bc stop token must have stop tag
    # print('termination')
    maxScore = float('-inf')
    finalTokenProb = None
    V.append({})
    for tag in tagset:
        # score from transitioning from tag to <stop> from last position  
        currentScore = V[input_length-3][tag] + score('<STOP>', tag, input_length-1)
        if currentScore > maxScore:
            maxScore = currentScore
            finalTokenProb = tag
    V[input_length-2]['<STOP>'] = maxScore
    backpointer.append({'<STOP>': finalTokenProb})
    
    # finding the best backpath, which we store as a sequence of tags
    # so that it fits the output type

    path = ['<STOP>']
    # firs tstart with stop, then we just build up the backpath
    # since we already store the highest score as part of each
    # we count down to 1, 
    # print('finding backpath')

    
    # what a pain in the butt
    # don't forget - input length includes start and stop tokens
    # tagset is the one that doesn't include start and stop tokens

    
    for i in range(input_length-2, 0, -1):
        # grab the best prev tags from backpointer table
        # find it by using the freshly inserted tag from previous iteration
        # backpointer[i][path[0]] represents best previous tag for current tag
        path.insert(0, backpointer[i][path[0]])
    # return ["<START>"] + [tagset[0]] * (input_length - 2) + ["<STOP>"]
    return ['<START>'] + path
...
def train(data, feature_names, tagset, epochs):
    ...
    def perceptron_gradient(i):
        ...
        def score(currentTag, pre_tag, i):
            return parameters.dot_product(features.compute_features(currentTag, pre_tag, i))
...
def test_decoder():
    ...
    def score_wrap(currentTag, pre_tag, i):
        retval = score(currentTag, pre_tag, i)
        print('Score('+currentTag+','+pre_tag+','+str(i)+') returning '+str(retval))
        return retval
    ...
    def score(currentTag, pre_tag, i):
        ...
        if i == 1:
            ...
            if currentTag == 'VB':
                return 4
        if i == 2:
            if currentTag == 'NN' and pre_tag == 'NN':
                return 4
            if currentTag == 'NN' and pre_tag == 'VB':
                return 9
            if currentTag == 'VB' and pre_tag == 'NN':
                return 5
            if currentTag == 'VB' and pre_tag == 'VB':
                return 0
        if i == 3:
            if currentTag != '<STOP>':
                print('ERROR: Current tag at i = 3 should be <STOP>. Current tag = '+currentTag)
            if pre_tag == 'NN':
                return 1
            if pre_tag == 'VB':
                return 1
...
def main_predict(data_filename, model_filename):
    ...
    feature_names = ['tag', 'prevTag', 'current_word']
...
class Features(object):
    ...
    def compute_features(self, currentTag, pre_tag, i):
        """
        Computes the local features for the current tag, the previous tag, and position i
        :param currentTag: String.  The current tag.
        ...
        if 'tag' in self.feature_names:
            feats.times_plus_equal(1, FeatureVector({'t='+currentTag: 1}))
        if 'prevTag' in self.feature_names:
            feats.times_plus_equal(1, FeatureVector({'ti='+currentTag+"+ti-1="+pre_tag: 1}))
        if 'current_word' in self.feature_names:
            feats.times_plus_equal(1, FeatureVector({'t='+currentTag+'+w='+self.inputs['tokens'][i]: 1}))
        return feats
